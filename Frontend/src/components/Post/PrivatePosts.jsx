...
const PrivatePosts = () => {
  ...
  useEffect(() => {
    ...
    // dispatch(resetPostAction()); //?clear old data
  }, [dispatch]);

  const getDate = (createdAt) => {
    const date = new Date(createdAt);
    const options = { day: "2-digit", month: "short", year: "numeric" };
    return date.toLocaleDateString("en-GB", options); // e.g. "13 Sep 2025"
  };
            ...
            posts?.allPosts?.map((post) => (
              <div key={post?._id} className="w-full md:w-1/2 px-4 mb-8">
                ...
                <p className="mb-2 text-coolGray-500 font-medium">
                  {post?.author?.username} • {getDate(post?.createdAt)}
                </p>
                <h4 className="inline-block mb-4 text-2xl md:text-3xl leading-tight text-coolGray-800 font-bold hover:underline">
                  {post?.title}
                </h4>
                <p className="mb-4 text-coolGray-500 line-clamp-3">
                  {post?.content}
                </p>
                <Link
